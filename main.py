...
# --- KONFIGURASI SMOOTHING (UPDATED) ---
# 1. Setting Wajah (Tetap rendah agar stabil/tidak getar)
HEAD_MIN_CUTOFF = 0.05 
HEAD_BETA       = 1.5  

# 2. Setting Badan/Tangan (Nilai TINGGI agar responsif/snappy)
BODY_MIN_CUTOFF = 1.0  # Naikkan ini biar tidak delay (sebelumnya 0.03)
BODY_BETA       = 4.0  # Naikkan ini biar cepat mengejar gerakan cepat

# 3. Setting Jari & Mata
FINGER_MIN_CUTOFF = 0.8 
FINGER_BETA       = 1.2 
EYE_MIN_CUTOFF    = 0.03  
EYE_BETA          = 1.0
...
ARM_GAIN_XY, ARM_GAIN_Z = 1.0, 0.35  # Gain XY dinaikkan ke 1.0 agar jangkauan tangan lebih pas
FINGER_SENSITIVITY = 0.9 
NECK_RATIO = 0.4 
...
# --- SETUP FILTERS ---
t_start = time.time()
# Filter Kepala (gunakan HEAD settings)
...
# Filter Spine (gunakan BODY settings agar badan ikut responsif)
...
# Per-bone quaternion filters to smooth bone rotations
# UPDATED: Menggunakan BODY_MIN_CUTOFF dan BODY_BETA agar tangan/kaki responsif
bone_names = [
...
for name in bone_names:
    bone_quat_filters[name] = [OneEuroFilter(t_start, 0, min_cutoff=BODY_MIN_CUTOFF, beta=BODY_BETA) for _ in range(4)]

# Quaternion filters for head and neck bones (tetap halus untuk wajah)
...
print("=== VTuber RESPONSIVE MODE: ON ===")
print(f"Body Smoothing: Cutoff={BODY_MIN_CUTOFF}, Beta={BODY_BETA} (Snappy)")
print(f"Face Smoothing: Cutoff={HEAD_MIN_CUTOFF}, Beta={HEAD_BETA} (Smooth)")

# Quick runtime check
try:
    build_info = cv2.getBuildInformation()
    if ('Win32 UI' not in build_info) and ('WIN32UI' not in build_info) and ('Cocoa' not in build_info and 'GTK' not in build_info):
        print('\nERROR: The OpenCV build in use does not include GUI support.')
        sys.exit(1)
except Exception:
    pass
...
while cap.isOpened():
    ...
    if results.face_landmarks:
        ...
        # Compute raw quaternions, then smooth per-component
    ...
    cv2.imshow('VTuber RESPONSIVE MODE', image)
